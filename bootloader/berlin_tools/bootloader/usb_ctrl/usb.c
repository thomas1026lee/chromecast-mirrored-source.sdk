	...
	while (index + 1 < dev->config.desc.wTotalLength) {
		...
		case USB_DT_ENDPOINT:
			...
			memcpy(&dev->config.if_desc[ifno].ep_desc[epno],
				&buffer[index], buffer[index]);
			le16_to_cpu(&(dev->config.if_desc[ifno].ep_desc[epno].\
							       wMaxPacketSize));
...
#if 0
	int i,j;
	printf("first time get descriptor.\n");
	for(i=0;i<512;i++,j++)
	{   if(j%32==0)
			printf("\n");
		printf("%2x",((unsigned char *)desc)[i]);
	}
	printf("\n");
#endif
...
#if 0
	printf("sec time get descriptor.\n");
	for(i=0;i<512;i++,j++)
	{   if(j%32==0)
			printf("\n");
		printf("%2x",((unsigned char *)&(dev->descriptor))[i]);
	}
	printf("\n");
#endif
	/* correct le values */
	le16_to_cpu(&dev->descriptor.bcdUSB);
...
static void usb_hub_power_on(struct usb_hub_device *hub)
{
	...
	for (i = 0; i < dev->maxchild; i++) {
		...
		USB_HUB_PRINTF("port %d returns %X\n", i + 1, dev->status);
...
#if 0
	int j;
	printf("first time get hub descriptor.\n");
...
#if 0
	printf("Sec time get hub descriptor.\n");
	for(i=0,j=0;i<512;i++,j++)
	{   if(j%32==0)
			printf("\n");
		printf("%2x",(unsigned char*)buffer[i]);
